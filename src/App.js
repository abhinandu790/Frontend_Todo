...
function App() {
  ...
  const BASE_URL = "https://backend-todo-ey6a.onrender.com";
  ...
  const MainApp = () => (
    <div className="app-container">
      ...
      <main className="main-content">
        ...
        <div className="filter-container">
          <select onChange={(e) => setFilterStatus(e.target.value)} className="filter-select" value={filterStatus}>
            <option value="all">All Status</option>
            <option value="pending">Pending</option>
            <option value="completed">Completed</option>
          </select>
          <select onChange={(e) => setFilterPriority(e.target.value)} className="filter-select" value={filterPriority}>
            <option value="all">All Priorities</option>
            <option value="low">Low</option>
            <option value="medium">Medium</option>
            <option value="high">High</option>
        ...
        <ul className="task-list">
          {filteredTasks.map((task) => (
            <li key={task._id} className="task-item">
              <div className="task-info">
                <span className="task-text">{task.text}</span>
                <span className="task-details">({task.status}, {task.priority})</span>
              </div>
              <div className="task-actions">
                <button
                  onClick={() => updateTaskStatus(task._id, task.status)}
                  className={`update-status-btn ${task.status === "pending" ? "pending-btn" : "completed-btn"}`}
                >
                  {task.status === "pending" ? "Mark Complete" : "Mark Pending"}
                </button>
                <select
                ...
                <button onClick={() => deleteTask(task._id)} className="delete-task-btn">Delete</button>
              </div>
            </li>
          ))}
        </ul>
